...
export function afficherPhotoRecette(moment, photoUrl, isCommunaute) {
...
export function prendrePhoto(moment) {
  ...
  input.onchange = async function() {
    ...
    var btn  = document.getElementById('photo-btn-' + moment)
    ...
    try {
      ...
      var doUpload = async function(consentPartage) {
        var url = await uploadPhoto(blob, moment)
        afficherPhotoRecette(moment, url, false)
      ...
      afficherModalConsentement(
        function() { doUpload(true)  },   // Partager
        function() { doUpload(false) }    // Garder privée
      )
    } catch(e) {
